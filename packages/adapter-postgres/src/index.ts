    ...
    implements IDatabaseCacheAdapter
{
    ...
    constructor(connectionConfig: Partial<pg.PoolConfig>) {
        super({
    ...
    // biome-ignore lint/suspicious/noExplicitAny: allow multiple result types
    ...
    async getAccountById(userId: UUID): Promise<Account | null> {
        return this.withDatabase(async () => {
            const { rows } = await this.pool.query('SELECT * FROM accounts WHERE id = $1', [
                userId,
            ]);
            if (rows.length === 0) {
    ...
    async getMemoriesByIds(memoryIds: UUID[], tableName?: string): Promise<Memory[]> {
        return this.withDatabase(async () => {
            ...
            const queryParams: string[] = [...memoryIds];
    ...
    async getMemories(params: {
        ...
        return this.withDatabase(async () => {
            ...
            const values: (string | number)[] = [params.tableName, params.roomId];
            let paramCount = 2;
                    ...
                    params.start || params.end
                        ? {
                              start: params.start
                                  ...
                                  : undefined,
                              end: params.end ? new Date(params.end).toISOString() : undefined,
                          }
                        : undefined,
                limit: params.count,
            });
    ...
    async getGoals(params: {
        ...
        return this.withDatabase(async () => {
            ...
            const values: (string | number)[] = [params.roomId];
            let paramCount = 1;
    ...
    async removeRoom(roomId: UUID): Promise<void> {
        ...
        return this.withDatabase(async () => {
            ...
            try {
                ...
                const checkResult = await client.query('SELECT id FROM rooms WHERE id = $1', [
                    roomId,
                ]);
    ...
    async searchMemoriesByEmbedding(
        ...
        return this.withDatabase(async () => {
            ...
            const values: (string | number)[] = [vectorStr, params.tableName];
    ...
    async removeMemory(memoryId: UUID, tableName: string): Promise<void> {
        return this.withDatabase(async () => {
            await this.pool.query('DELETE FROM memories WHERE type = $1 AND id = $2', [
                tableName,
                memoryId,
            ]);
        }, 'removeMemory');
    ...
    async removeAllMemories(roomId: UUID, tableName: string): Promise<void> {
        return this.withDatabase(async () => {
            await this.pool.query(`DELETE FROM memories WHERE type = $1 AND "roomId" = $2`, [
                tableName,
                roomId,
            ]);
        }, 'removeAllMemories');
    ...
    async deleteCache(params: {
        ...
        return this.withDatabase(async () => {
            try {
                ...
                try {
                    await client.query('BEGIN');
                    await client.query(`DELETE FROM cache WHERE "key" = $1 AND "agentId" = $2`, [
                        params.key,
                        params.agentId,
                    ]);
                    await client.query('COMMIT');
    ...
    async getKnowledge(params: {
        ...
        return this.withDatabase(async () => {
            ...
            const queryParams: (string | number)[] = [params.agentId];
            let paramCount = 1;
        ...
        return this.withDatabase(async () => {
            ...
            try {
                ...
                if (typeof id === 'string' && id.includes('-chunk-*')) {
                    ...
                    await client.query('DELETE FROM knowledge WHERE "originalId" = $1', [mainId]);
                } else {
                    // First delete all chunks associated with this knowledge item
                    await client.query('DELETE FROM knowledge WHERE "originalId" = $1', [id]);
    ...
    private async createKnowledgeChunk(params: {
        ...
        originalId: UUID;
        agentId: UUID | null;
        // biome-ignore lint/suspicious/noExplicitAny: allow any content type
        content: any;
